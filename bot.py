...
async def on_message(message):
    ...
    if not(aux.checkmod(message)):
        ...
        elif str(message.channel.id) in [config['WHOLESOME_MEMES_ID'], config['ART_SHARE_ID'], config['GLITCHTOBER_ID'], "910570529679888416"]:
...
async def create_ticket_channel(init_message,name,user):
    ...
    if name == "merch-ticket":
        overwrites[merch_support_role] = discord.PermissionOverwrite(read_messages=True, manage_channel=True)
